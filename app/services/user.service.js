...
exports.forgetPassword = async function (req, res) {
    ...
    try {
        ...
        if (userExist) {
            ...
            res.status(400).send({
                msg: "The email address you entered is not register with us.",
            });
        }
    } catch (error) {
        res.send(error);
...
exports.updatePassword = function (req, res) {
    ...
    Token.findOne({ token: req.body.token }, function (err, token) {
        if (!token) {
            return res
                .status(400)
                .send({ type: "not-verified", msg: "your link has been expired" });
        }
        ...
        User.findOne(
            ...
            async function (err, user) {
                if (!user) {
                    return res
                        .status(400)
                        .send({ msg: "We are unable to find the user for this Link" });
                }
